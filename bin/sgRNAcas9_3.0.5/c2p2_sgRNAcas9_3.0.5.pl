...
mkdir("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname",0755)||die "Can't create directory: Directory exists at $dir. Please delete, move or rename the exist directory before you run this program.$!" ;

open  (LOG, ">>$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/sgRNAcas9.Log.txt") || die "Can't open sgRNAcas9.Log.txt for writing!" ."\n";
print  LOG "################################# Log ###########################################".        "\n\n";
...
#mkdir("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/A.Final_report",0755)||die "can't create directory: $!" ;
...
open (FASTA, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/TargetSeq.fa") || die "Can't open TargetSeq.fa for writing!\n";
...
open(IntPut, "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/TargetSeq.fa") || die "Can't open TargetSeq.fa for reading!\n";
#report_protospacer_single.txt为CRISPR.targets_S.txt和CRISPR.targets_A.txt文件合并结果。
open(OutPut, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_protospacer_single.txt") || die "Can't open report_protospacer_single.txt for writing!\n";
open(OutPut1, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa") || die "Can't open CRISPR.targets_single.fa for writing!\n";
#open(OutPut2, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/full_length_sgRNA.txt") || die "Can't open full_length_sgRNA.txt for writing!\n";
open(OutPut3, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_S.txt") || die "Can't open CRISPR.targets_single.fa for writing!\n";
open(OutPut4, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_A.txt") || die "Can't open CRISPR.targets_single.fa for writing!\n";
...
open( PA, "<$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_A.txt" ) || die "can't open CRISPR.targets_A.txt!";
open( PS, "<$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_S.txt" ) || die "can't open CRISPR.targets_S.txt!";
open( PAIRS1, ">>$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_protospacer_pairs.xls" ) || die "can't open report_protospacer_pairs.xls!";
open( PAIRS2, ">>$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_pairs.fa" ) || die "can't open CRISPR.targets_pairs.fa!";
...
while ( <PA> ) {
	...
	next if $len_target_seq_A ne ($truncat+3);  #长度相同则继续后面程序，不等就退出；cas9和c2p2因PAM长度不同，在此处需要修改后面数字
...
my %seq;
my $title;
my $infile="$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_pairs.fa";
...
open (OUT,">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa");
foreach (@uniqueseq){
	print OUT ">$seq{$_}\n$_\n";
}
close OUT;
...
if ($Seqmap_vesion eq "l" and $Type eq "s") {       #Linux-64
	...
	my @args = ("./Seqmap/seqmap-1.0.12-linux-64","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-linux-64","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-windows.exe","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-windows.exe","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-mac-64","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-mac-64","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-mac","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-mac","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-linux","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
	...
	my @args = ("./Seqmap/seqmap-1.0.12-linux","$Num_mismatch", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa", "$Genome", "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt", "/output_all_matches");
	system(@args);
...
open(Input, "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt") || die "Can't open seqmap_output.txt for reading!\n";
open(Out, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_sgRid.txt") || die "Can't open report_sgRid.txt for writing!\n";
...
while (<Input>) {
	...
	next if ($trans_id eq "trans_id" or (substr($target_seq,$len1,1) ne "T") or (substr($target_seq,$len2,1) ne "T") ) ;
    #                 匹配到表头                          第一个碱基不是T                          第二个碱基不是T 
...
mkdir("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/A.Sort_OT_byID",0755)||die "can't create directory: $!" ;
...
my $len4= $truncat-20; #0
my $len5= $truncat-19; #1

my $len7= $truncat-18; #2

open(Seq, "<$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt" ) || die "can't open seqmap_output.txt!";
open(sgR, "<$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_sgRid.txt" ) || die "can't open report_sgRid.txt!";

open(Out3, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/search_OT.txt") || die "Can't open search_OT.txt for writing!\n";
...
while (<Seq>) {
	...
	while (<sgR>) {
		...
		#next if 为假时只是退出某一次循环 
   		#discard  以下主要是为了去掉TTN的PAM区错配比对的基因组比对结果
	    next if ($trans_id eq "trans_id" or (substr($target_seq,$len4,1) ne "T") or (substr($target_seq,$len5,1) ne "T") ) ;
		
			open (Out2,">>$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/A.Sort_OT_byID/$sgR_ID.txt");
			
	   #  处理例如TTA和TTG这种PAM区的错配                   TT-[N]                                        
	    if($num_mismatch > 0 and (substr($target_seq,$len7,1) ne substr($probe_seq,$len7,1))){
	
	    	$New_num_mismatch=$num_mismatch-1;  #TTN中N允许不同，所以这几种情况序列比对上的错配不算作靶标错配，故错配值减去1
...
sub analysis {  
	...
	for($idx_s=-1; $idx_s<length($seq); $idx_s++) {
		...
		my $lentruncat = $truncat+3;  #23
		my $lentruncat2 = $truncat+1; #21

		if (substr($SEQDNA, $idx_s+1, $lentruncat) =~ /TT[ATCG]{$lentruncat2}/) {    #TTN-X?  
			my $sgRidx_s = $idx_s+2;                                  #For sense strand,sgRidx_s表达的是所匹配sgRNA序列的起始位置
			my $sgR = $&;  # $& 表示与格式匹配的字符串,此处即表示匹配到的27bp的sgRNA序列
			#print $sgR."\n";
			my $SgR1 = substr($sgR,3,21);   #去掉PAM区TTN的前两个TT碱基
			#print $SgR1."\n";
			...
			my $SgRNt = substr($sgR,4,$truncat);                      #truncat序列，TTTN后面的20个碱基
...
########count_ot#########
my $Dir3_ot = "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/A.Sort_OT_byID" ;
...
open(OutPut, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_count_total_ot.txt") || die "Can't open report_count_total_ot.txt for writing!\n";

print OutPut "ID\t0M(on-/off-)\t1M\t2M\t3M\t4M\t5M\tTotal OT number\n";  # c2p2_$faname_sgRNAcas9_report.xls结果中OT统计结果
...
#########
#########
my $File1_ot = "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_protospacer_single.txt";
my $File2_ot = "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_count_total_ot.txt";
...
open( PAIRS1, ">$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/single_and_ot.txt" ) || die "can't open single_and_ot.txt!";
...
#########
my $File1 = "$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/single_and_ot.txt";
...
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/seqmap_output.txt")||die "Can't delete seqmap_output.txt file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/search_OT.txt")||die "Can't delete search_OT.txt file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/unique_pairs.fa")||die "Can't delete unique_pairs.fa file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_A.txt")||die "Can't delete CRISPR.targets_A.txt file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_S.txt")||die "Can't delete CRISPR.targets_S.txt file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_pairs.fa")||die "Can't delete CRISPR.targets_pairs.fa file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/CRISPR.targets_single.fa")||die "Can't delete CRISPR.targets_single.fa file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_sgRid.txt")||die "Can't delete report_sgRid.txt file";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_protospacer_single.txt")||die "Can't delete report_protospacer_single.txt";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/report_count_total_ot.txt")||die "Can't delete report_count_total_ot.txt";
#unlink ("$dir/c2p2.sgRNAcas9.report_$truncat.$Option.$faname/single_and_ot.txt")||die "Can't delete single_and_ot.txt";
...
print "Your job is done, open c2p2.sgRNAcas9.report_$truncat.$Option directory to check result.". "\n";
print  LOG "# Your job is done, open c2p2.sgRNAcas9.report_$truncat.$Option directory to check result.". "\n";
