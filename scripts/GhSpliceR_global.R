...
# Given a chromosome and genomic coordinates this function extracts genomic sequences. 1.12.18
get_transcript = function(id, upstream = 0, downstream = 0){
  require(magrittr)
  require(tidyverse)
  library("TxDb.GossypiumhirsutumGFF.HZAU.Ghirsutum")
  txdb <- TxDb.GossypiumhirsutumGFF.HZAU.Ghirsutum
  cds <- cdsBy(txdb, by="tx", use.names=TRUE)
  tibble(as.data.frame(cds)) %>%
    dplyr::filter(group_name == id) %>%
    dplyr::select(exon_chrom_start = start,
                  exon_chrom_end = end,
                  strand = strand,
    ...
    dplyr::mutate(gene_id = gsub("\\..*", "", gene_id),
                  ...
                  strand = as.numeric(paste0(strand, 1)))
}

### Function for getting sequence from genomic coordinates
get_seq = function(start, end, strand, chromosome, upstream = 0, downstream = 0){
  require(magrittr)
  require(tidyverse)
  require(Biostrings)
  require(Rsamtools)
  ...
  seq = if(strand == 1) {
    ...
    getSeq(genome, chromosome, start, end)
  } else {
    anti_end = start + upstream
    anti_start = end - downstream
    getSeq(genome, chromosome, anti_start, anti_end)
  }
  return(seq)
}
...
generate_guides = function(ensembl_id, pam){
  ...
  gene = get_transcript(transcript)
  sense = gene$strand[1]
  ...
  {# Keep in mind this is in reverse orientation and the ends really signify the starts of exons.
    ...
    gene_seq = get_seq(start = gene$exon_chrom_end[1],
                       end = gene$exon_chrom_start[exon_number],
                       strand = gene$strand[1],
                       chromosome = gene$chromosome_name[1],
                       upstream = flank + pam_length,
                       downstream = downstream_gene_seq + pam_length
  ...
  if(length(guides$exon) > 0) { # Edited 
    ...
    guides$splice_site = as.character(guides$splice_site)
    guides = gene %>% 
    ...
    data = guides %>% 
      ...
      mutate(., 
             total_be_score = 100*((
               (be_motif_score/2)*motif_weight + 
                 be_efficiency/max(be_efficiencies$be_efficiency)*efficiency_weight +
      ...
      mutate(., 
             total_abe_score = 100*((
               abe_efficiency/max(abe_efficiencies$abe_efficiency)*efficiency_weight +
                 position_score*position_weight)
